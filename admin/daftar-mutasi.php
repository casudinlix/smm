...
if (isset($_GET['tampil'])) {
    ...
    $cek_riwayat = "SELECT * FROM mutasi WHERE aksi LIKE '%$cari_aksi%' ORDER BY id DESC"; // edit
} else {
...
while ($data_riwayat = $new_query->fetch_assoc()) {
    if ($data_riwayat['status'] == "UNREAD") {
        $label = "danger";
    } else if ($data_riwayat['status'] == "READ") {
        $label = "success";
    }
?>
                                <tr>
                                    ...
                                    <td width="10%"><span class="badge badge-<?php echo $label; ?>"><?php echo $data_riwayat['status']; ?></span></td>
