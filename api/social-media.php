                        ...
                        if (mysqli_num_rows($cek_layanan) == 0) {
                            ...
                            if ($jumlah < $data_layanan['min']) {
                                ...
                                if ($provider == "MANUAL") {
                                    ...
                                    $post_datanya = "api_token=".$data_provider['api_key']."&action=add&package=".$data_layanan['provider_id']."&link=$target&quantity=$jumlah";
                                    $url = "https://bospanel.com/api/v2";
                                } else if ($provider == "FOLLOWIZ"){
                                    $post_datanya = "key=".$data_provider['api_key']."&action=add&service=".$data_layanan['provider_id']."&link=$target&quantity=$jumlah";
                                ...
                                if ($provider == "IRVANKEDE" AND $resultnya['status'] == false) {
                                    $hasilnya = array('status' => false, 'data' => array('pesan' => 'Ups, Server Kami Sedang Mengalami Gangguan, Silahkan Di Coba Lagi Nanti.'));
                                } else if ($provider == "MEDANPEDIA" AND $resultnya['status'] == false) {
                                    $hasilnya = array('status' => false, 'data' => array('pesan' => 'Ups, Server Kami Sedang Mengalami Gangguan, Silahkan Di Coba Lagi Nanti.'));
                                } else if ($provider == "BOSPANEL" AND $resultnya['order'] == false) {
                                    $hasilnya = array('status' => false, 'data' => array('pesan' => 'Ups, Server Kami Sedang Mengalami Gangguan, Silahkan Di Coba Lagi Nanti.'));
                                } else if ($provider == "FOLLOWIZ" AND $resultnya['order'] == false) {
                                    $hasilnya = array('status' => false, 'data' => array('pesan' => 'Ups, Server Kami Sedang Mengalami Gangguan, Silahkan Di Coba Lagi Nanti.'));
                                } else {
                                    ...
                                    if ($provider == "IRVANKEDE") {
                				        ...
                				        $provider_oid = $resultnya['order'];
                				    } else if($provider == "FOLLOWIZ") {
                				        $provider_oid = $resultnya['order'];
                				    }
 
                                    if ($conn->query("INSERT INTO pembelian_sosmed VALUES ('','$order_id', '$provider_oid', '".$datanya['username']."', '".$data_layanan['layanan']."', '$target', '$jumlah', '0', '$start_count', '$harga', '$profit', '$koin', 'Pending', '$date', '$time', '$provider', 'API', '0')") == true) {
                                        ...
                                        $conn->query("INSERT INTO riwayat_saldo_koin VALUES ('', '".$datanya['username']."', 'Pengurangan Saldo', '$harga', 'Pemesanan Sosial Media Via API Dengan Kode Pesanan : $order_id', '$date', '$time')"); 
                                        $conn->query("INSERT INTO semua_pembelian VALUES ('','$order_id', '$order_id', '".$datanya['username']."', '".$data_layanan['kategori']."', '".$data_layanan['layanan']."', '$harga', '$target', 'Pending', '$date', '$time', 'API', '0')");
